...
object DeviceAuthenticationPrompt {
    /**
     * Launch a system prompt requiring the user to authenticate their Android device. On APIs lower
     * than 30, this has to support the password and biometrics (if user configured them). On 30+,
     * it can be toggled to disallow biometrics
     */
    fun openPrompt(activity: FragmentActivity, callback: BiometricPrompt.AuthenticationCallback) {
        ...
        val appName =
            if (Prefs.isCamoEnabled) Prefs.camoAppDisplayName ?: ""
    ...
    /**
     * Returns null if the device is configured to do the authentication prompt or else a
     * user-facing String indicating that the device is not configured or otherwise fundamentally
    ...
    /**
     * Get legal BiometricManager.Authenticators for different versions of Android if on API 30+
     * this is configured with a preference, otherwise its hardcoded
     */
